...
func (stmt *BackendStatement) readPrepareResultPacket() (uint16, error) {
	data, err := stmt.conn.c.readEphemeralPacket()
	if err != nil {
		return 0, err2.NewSQLError(mysql.CRServerLost, mysql.SSUnknownSQLState, "%v", err)
	}
	defer stmt.conn.c.recycleReadPacket()

	// packet indicator [1 byte]
	if data[0] != mysql.OKPacket {
...
func (stmt *BackendStatement) writeCommandLongData(paramID int, arg []byte) error {
	...
	for argLen := len(arg); argLen > 0; argLen -= pktLen - dataOffset {
		...
		stmt.conn.c.sequence = 0
		// Add command byte [1 byte]
		data[4] = mysql.ComStmtSendLongData
		...
		// Send CMD packet
		err := stmt.conn.c.writePacket(data[:4+pktLen])
		if err == nil {
